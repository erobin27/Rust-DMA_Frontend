...
import { RecoilComponent } from "./recoil.component";

type DataType = "Data" | "Ids" | "Remove Ids";

const DataScene: React.FC = () => {
  const { messagesPerSecond, data, localPlayerData, send, ids, removeIds, isConnected, url, closeConnection } =
    useWebSocket();
  ...
  return (
    <Box sx={{ padding: "30px", width: "100%" }}>
      ...
      {/* RECOIL BOX */}
      <Box mt={'50px'}>
        <RecoilComponent sendToWebsocket={send} weaponName={localPlayerData?.holding.name || 'none'} current={localPlayerData?.holding.recoil}/>
      </Box>
